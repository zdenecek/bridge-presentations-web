    ...
    <CardProvider :game="game">
      <div class="main-view" ref="mainView">
        <OneDimensionalHandView ref="handViewWest" :hand="game?.players[Position.West].hand" :position="Position.West"
          :rotation="Rotation.Left"></OneDimensionalHandView>
        <div class="center-column">
          <OneDimensionalHandView ref="handViewNorth" :hand="game?.players[Position.North].hand"
            :position="Position.North" :rotation="Rotation.Top"></OneDimensionalHandView>
          ...
          <OneDimensionalHandView ref="handViewSouth" :hand="game?.players[Position.South].hand"
            :position="Position.South" :rotation="Rotation.Top"></OneDimensionalHandView>
        </div>

        <OneDimensionalHandView ref="handViewEast" :hand="game?.players[Position.East].hand" :position="Position.East"
          :rotation="Rotation.Right"></OneDimensionalHandView>
        <DebugView></DebugView>
      </div>
...
import { onMounted, ref, useTemplateRef, watch, provide, triggerRef, onUnmounted, computed } from 'vue';
...
watch(() => props.game, (game) => {
    ...
    game.allPlayers.forEach((player) => {
      player.bidRequested.sub(() => {
        biddingBoxCallback.value = (bid) => (player as PresentationPlayer).bid(bid);
      });
...
const handViewWest = useTemplateRef<typeof OneDimensionalHandView>('handViewWest')
const handViewNorth = useTemplateRef<typeof OneDimensionalHandView>('handViewNorth')
const handViewSouth = useTemplateRef<typeof OneDimensionalHandView>('handViewSouth')
const handViewEast = useTemplateRef<typeof OneDimensionalHandView>('handViewEast')
