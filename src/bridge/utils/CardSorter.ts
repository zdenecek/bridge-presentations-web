...
export function sortCardsInHandBySuitOrder(
...
export function sortCardsBySuitOrder(
  cards: Array<Card>,
  order: Array<Suit>,
): void {
...
export function sortCardsInHandByPrioritizedSuit(
  cards: Array<CardInHand>,
  suit: Suit,
): void {
  _sortCards(
    cards,
    sortSuits(defaultOrder, suit),
    (card: CardInHand) => card.card,
  );
}

export function sortCardsByPrioritizedSuit(
  cards: Array<Card>,
  suit: Suit,
): void {
...
export function sortSuits(
  suits: Array<Suit>,
  prioritizedSuit: Suit | undefined = undefined,
): Array<Suit> {
  const new_order = Object.assign([], defaultOrder);

  if (prioritizedSuit) {
    while (new_order[0] != prioritizedSuit)
      new_order.unshift(new_order.pop() ?? errorMessage("Error in sortSuits"));
  }
